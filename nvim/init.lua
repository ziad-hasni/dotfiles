...
-- ============================================================
-- SECTION 8a: My plugins
-- ============================================================
do
	vim.pack.add({ gh("akinsho/toggleterm.nvim") })
	require("toggleterm").setup({ open_mapping = [[<c-c>]], direction = "float" })

	vim.pack.add({ gh("kdheepak/lazygit.nvim") })
	vim.keymap.set("n", "<leader>lg", "<cmd>LazyGit<cr>", { desc = "LazyGit" })

	vim.pack.add({ gh("nvim-tree/nvim-tree.lua") })
	require("nvim-tree").setup({ view = { side = "left" }, hijack_netrw = false, disable_netrw = false })

	vim.pack.add({ gh("tikhomirov/vim-glsl") })

	vim.pack.add({ gh("mfussenegger/nvim-dap") })
	vim.pack.add({ gh("rcarriga/nvim-dap-ui") })
	vim.pack.add({ gh("nvim-neotest/nvim-nio") })

	vim.pack.add({ gh("ellisonleao/carbon-now.nvim") })
	...
	vim.pack.add({ gh("FabianWirth/search.nvim") })
